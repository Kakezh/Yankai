import { login, register, bind, unfreeze, logout, getPermissionList, getInfo, getUserPermissionList } from '@/api/user'
...
const actions = {
  ...
  // user unfreeze
  unfreeze({ commit }, userInfo) {
    const { username, phonenumber, mailnumber } = userInfo
    return new Promise((resolve, reject) => {
      unfreeze({ username: username.trim(), phonenumber: phonenumber, mailnumber: mailnumber })
        .then(response => {
          resolve()
        })
        .catch(error => {
  ...
  bind({ commit }, userInfo) {
    const { username, phonenumber, mailnumber } = userInfo
    return new Promise((resolve, reject) => {
      bind({ username: username.trim(), phonenumber: phonenumber, mailnumber: mailnumber })
        .then(response => {
          resolve()
        })
        .catch(error => {
