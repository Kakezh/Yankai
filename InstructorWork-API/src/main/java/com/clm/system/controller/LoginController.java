...
public class LoginController {
    ...
    public Result<UserVo> login(@RequestBody LoginModel loginModel){
        ...
        System.out.println(sysUserService.getErrorNum(loginModel.getUsername())+"=======================");
        ...
        if (!sysUser.getPassword().equals(inputPassword)){
            if(sysUserService.getErrorNum(username)==2){
                result.error20001("登录失败次数达上限，账户已锁定！");
                sysUserService.freezeUser(username);
                return result;
            }
            sysUserService.setErrorNum(username);
