...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    @Override
    public Result<?> checkRegisterUserIsEffective(SysUser sysUser){
        Result<Object> result = new Result<>();
        if(sysUser!=null){
            result.error20001("账户已存在");
        }
        return result;
    }

    @Override
    public Result<?> checkUnfreezeUserIsEffective(SysUser sysUser){
        Result<Object> result = new Result<>();
        //情况1：根据用户信息查询，该用户不存在
        if (sysUser == null) {
            result.error20001("该用户不存在，请注册");
    ...
    @Override
    public int getErrorNum(String username){
        return sysUserMapper.getErrorNum(username);
    }

    @Override
    public boolean setErrorNum(String username){
        int tempNum=getErrorNum(username);
        return sysUserMapper.setErrorNum(username);
    }

    @Override
    public boolean clearErrorNum(String username){
        return sysUserMapper.clearErrorNum(username);
    }

    @Override
    public boolean freezeUser(String username){
        return sysUserMapper.freezeByUsername(username);
    }

    @Override
    public boolean unfreezeUser(String username){
        return sysUserMapper.unFreezeByUsername(username);
    }

    @Override
    public boolean bindPhoneAndMail(UnfreezeModel bindModel){
        if(bindModel.getPhonenumber()==""){
            bindModel.setPhonenumber(getUserInfoByUsername(bindModel.getUsername()).getPhone());
        }
        if(bindModel.getMailnumber()==""){
            bindModel.setMailnumber(getUserInfoByUsername(bindModel.getUsername()).getEmail());
        }
        return sysUserMapper.bindPhoneAndMail(bindModel);
    }
}
